...
<div class="body">
  ...
  <?php foreach ($mecanics as $mecanic) { ?>
      ...
      <?php $suma_mecanic = 0;
        foreach ($mecanic->Service->where('mecanic_id', '=', $mecanic->id)
                                  ->where('created_at','<', $to)
                                  ->where('created_at', '>', $from) as $value) {
            $suma_mecanic += $value->suma_totala;
        } ?>
        
        <th>
          <h3> 
            <?php if($suma_mecanic != 0){
                    echo $suma_mecanic . ' lei'; 
                  } else { 
                    echo '-';
                  } ?>
          </h3>
  ...
  <?php foreach ($reglarea_mecanics as $reglarea_mecanic) { ?>
    ...
    <?php $suma_reglare_unghi = 0;
       foreach ($reglarea_mecanic->ReglareaUnghiului->where('mecanic_id', '=', $reglarea_mecanic->id)
                                                    ->where('created_at', '<', $to)
                                                    ->where('created_at', '>', $from) as $suma_reglarii){
                $suma_reglare_unghi += $suma_reglarii->pret_lucru;  
              } ?>

      <th>
        <h3> 
          <?php if($suma_reglare_unghi != 0){
  ...
  <?php 
    $suma_serviciilor_vulcanizare = 0;
     foreach ($vulcanizator->Vulcanizare->where('vulcanizator_id', $vulcanizator->id)
                                        ->where('created_at', '<', $to)
                                        ->where('created_at', '>', $from) as $suma_vulcanizarii ) {
      $suma_serviciilor_vulcanizare += $suma_vulcanizarii->suma;
      } ?>

        <th>
          <h3> <?php if($suma_serviciilor_vulcanizare != 0){
  ...
  <tr>
    ...
    <tr>
      ...
      <td>
        <h3>
          <?php $total = $suma_vulcanizare_transfer+$suma_vulcanizare+$suma_totala_service;
            echo $total . ' lei'; ?>
        </h3>
      </td>
    </tr>
...
<?php $from1 = date( 'd/m/y', strtotime( $from ) );
$to1 = date( 'd/m/y', strtotime( $to ) );
?>
<h4 style="text-align: right;">Din data de: {{$from1}} - {{$to1}}</h4>
