...
<form method="POST" action="/vulcanizare/store">
  {{ csrf_field() }}
...
<select class="form-control" name='vulcanizator_id' data-live-search="true">
@foreach($vulcanizator as $vulcanizators)
     <option value='{{ $vulcanizators->id }}'>{{ $vulcanizators->nume }}</option>
@endforeach
     </select>
